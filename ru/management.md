...
> **Опасные команды:** сначала проверьте context и namespace. Удаление namespace удаляет всё его содержимое, а `--all` и `--force --grace-period=0` могут привести к простою или потере данных.
{: .warning}
...
> **Предупреждение об ownership:** `--force-conflicts` забирает конфликтующие поля у других field manager-ов. Перед применением проверьте `kubectl diff --server-side`.
{: .warning}
...
> **Предупреждение о пересоздании:** `kubectl replace --force` удаляет и создаёт ресурс заново. Изменится UID и возможен простой.
{: .warning}
