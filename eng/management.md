...
> **Destructive commands:** verify the context and namespace first. Deleting a namespace removes everything in it; `--all` and `--force --grace-period=0` can cause outages or data loss.
{: .warning}
...
> **Ownership warning:** `--force-conflicts` transfers conflicting fields from other field managers. Review `kubectl diff --server-side` before using it.
{: .warning}
...
> **Recreation warning:** `kubectl replace --force` deletes and recreates the resource. Expect a new UID and possible downtime.
{: .warning}
